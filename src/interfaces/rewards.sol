interface RewardsController {
    ...
    function allMarketsOperations()
        external
        view
        returns (MarketOperation[] memory marketOps);

    function allRewards() external view returns (address[] memory);

    function availableRewardsCount(
        address market
    ...
    function distribution(
        address
    ) external view returns (uint8 availableRewardsCount, uint256 baseUnit);
